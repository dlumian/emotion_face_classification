...
class EmotionFacePredictor():
    ...
    '''
    def __init__(self, home, cv2_path, model_path, duration=30):
        ...
        cv2.resizeWindow(self.winname, 2500,2500)
        cv2.moveWindow(self.winname, 800,30)  # Move it to (40,30)
    ...
    def classify_faces_video(self,file_path=0,duration=15, write_imgs=False, output_name='test', show_plots=True, show_final_plot=True):
        ...
        try:
            ...
            self.means_to_plot = np.vstack(self.total_df_probas).mean(0)
        ...
        plt.title('Overall Emotion Percentages')
...
if __name__=='__main__':
    ...
    efp = EmotionFacePredictor(home, cv2_path, bestmodelfilepath, duration=duration)
    efp.run_setup()
    # efp.classify_faces_image('./faces/face_174.jpg')
    print(f"Running demo for {efp.duration} seconds")
    efp.classify_faces_video(duration=efp.duration)
