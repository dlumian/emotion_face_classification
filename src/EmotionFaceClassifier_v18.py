...
class EmotionFaceClassifier():
    ...
    # def plot_pie_chart_of_categories(self):
    #     img = self.t1.groupby('emotion').size().plot.pie()
    ...
    def MNB_analysis(self):
        ...
        result_ser['Model'] ='MultinomialNB'
        result_ser['Balanced'] = 'No'
        self.MNB = MultinomialNB()
        self.MNB.fit(self.x_train_flat, self.y_train)
        train_pred_y = self.MNB.predict(self.x_train_flat)
        result_ser['Train Accuracy'] = accuracy_score(self.y_train, train_pred_y)
        self.MNB.pred_y_ = self.MNB.predict(self.x_test_flat)
        result_ser['Test Accuracy'] = accuracy_score(self.y_test, self.MNB.pred_y_)
    ...
    def MNB_bal_analysis(self):
        ...
        result_ser = pd.Series()
        result_ser['Model'] ='MultinomialNB'
        result_ser['Balanced'] = 'Yes'
        self.MNB_bal = MultinomialNB()
        self.MNB_bal.fit(self.bal_x_train_flat, self.bal_y_train)
        train_pred_y = self.MNB_bal.predict(self.x_train_flat)
        result_ser['Train Accuracy'] = accuracy_score(self.y_train, train_pred_y)
        self.MNB_bal.pred_y_ = self.MNB_bal.predict(self.x_test_flat)
        result_ser['Test Accuracy'] = accuracy_score(self.y_test, self.MNB_bal.pred_y_)
    ...
    def RF_analysis(self):
        ...
        result_ser = pd.Series()
        result_ser['Model'] ='Random Forest'
        result_ser['Balanced'] = 'No'
        ...
        train_pred_y = self.RF.predict(self.x_train_flat)
        result_ser['Train Accuracy'] = accuracy_score(self.y_train, train_pred_y)
        self.RF.pred_y_ = self.RF.predict(self.x_test_flat)
        result_ser['Test Accuracy'] = accuracy_score(self.y_test, self.RF.pred_y_)
    ...
    def cnn_analysis_y_cat(self, img_dimensions=(48,48,1)):
        result_ser = pd.Series()
        result_ser['Model'] ='CNN Categorical'
        result_ser['Balanced'] = 'No'
        self.n_classes = len(np.unique(self.y_train))
        ...
        train_pred_y = self.best_model.predict_classes(self.x_train)
        print(train_pred_y)
        result_ser['Train Accuracy'] = accuracy_score(self.y_train, train_pred_y)
        self.y_pred_cat = self.best_model.predict_classes(self.x_test)
        result_ser['Test Accuracy'] = accuracy_score(self.y_test, self.y_pred_cat)
        self.save_cm(self.y_pred_cat, 'Categorical_CNN')
        self.results_df = self.results_df.append(result_ser, ignore_index=True)

    def cnn_analysis_y_cont(self, img_dimensions=(48,48,1)):
        result_ser = pd.Series()
        result_ser['Model'] ='CNN Continuous'
        result_ser['Balanced'] = 'No'
        self.n_classes = len(np.unique(self.y_train))
        # self.batch_size = batch_size
        # self.model = create_model(img_dimensions, self.n_classes)
        ...
        result_ser['Test Accuracy'] = accuracy_score(self.y_test, self.y_pred_cont)
    ...
    def format_results_df(self):
        self.organized_results = self.results_df[['Model', 'Balanced','Train Accuracy','Test Accuracy']]
